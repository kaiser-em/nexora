...
use Silao\Blueprint\Exception\BlueprintException;
...
final class RestErrorMapper
{
    ...
    {
        if ($e instanceof BookingValidationException || $e instanceof InvalidFieldException || $e instanceof InvalidBlueprintException) {
            return new WP_Error('silao_rest_validation_failed', $e->getMessage(), ['status' => 422]);
        }

        if ($e instanceof BookingUnavailableException) {
            return new WP_Error('silao_rest_unavailable', $e->getMessage(), ['status' => 409]);
        }

        if ($e instanceof DuplicateSlugException) {
            return new WP_Error('silao_rest_slug_exists', $e->getMessage(), ['status' => 409]);
        }

        if ($e instanceof InvalidBookingException) {
            return new WP_Error('silao_rest_bad_transition', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof InvalidPricingContextException) {
            return new WP_Error('silao_rest_bad_pricing_context', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof InvalidPricingRuleException) {
            return new WP_Error('silao_rest_bad_pricing_rule', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof InvalidOptionException) {
            return new WP_Error('silao_rest_bad_option', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof InvalidBookingModelException) {
            return new WP_Error('silao_rest_bad_model', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof CurrencyMismatchException) {
            return new WP_Error('silao_rest_currency_mismatch', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof MoneyOverflowException) {
            return new WP_Error('silao_rest_money_overflow', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof BlueprintException) {
            return new WP_Error('silao_rest_bad_request', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof ServiceNotFoundException || $e instanceof BlueprintNotFoundException) {
            return new WP_Error('silao_rest_not_found', $e->getMessage(), ['status' => 404]);
        }

        if ($e instanceof ApplicationException) {
            return new WP_Error('silao_rest_application_error', $e->getMessage(), ['status' => 400]);
        }

        if ($e instanceof PersistenceException) {
            return new WP_Error('silao_rest_persistence_error', 'A database persistence error occurred. Please try again later.', ['status' => 500]);
        }

        return new WP_Error('silao_rest_internal_error', 'An unexpected internal server error occurred.', ['status' => 500]);
    }
}